...
class LanguageModel(object):
    ...
    def __init__(self, options, is_training):
        ...
        self._build()
    #todo word的embedding模型构造
    def _build_word_embeddings(self):
        n_tokens_vocab = self.options['n_tokens_vocab']#todo vocabulary的大小
        batch_size = self.options['batch_size']
        unroll_steps = self.options['unroll_steps']

        # LSTM options
        projection_dim = self.options['lstm']['projection_dim'] #todo 每个单词编码的embedding的大小
    ...
    #todo 创建字母的embedding模型
    def _build_word_char_embeddings(self):
        '''
        options contains key 'char_cnn': {

        'n_characters': 261,  #字符字典中总的字符个数
        ...
        '''
        batch_size = self.options['batch_size']
        unroll_steps = self.options['unroll_steps']#20  #最大时长,n_token
        projection_dim = self.options['lstm']['projection_dim']  #最终维度,投影层维度
    
        cnn_options = self.options['char_cnn']
        filters = cnn_options['filters']
        n_filters = sum(f[1] for f in filters) #一个卷积核一组特征，总的特征组数
        max_chars = cnn_options['max_characters_per_token']#todo 每个单词最大的长度
        char_embed_dim = cnn_options['embedding']['dim'] #todo 每个字符编码的长度
        ...
        # the convolutions
        # todo 卷积层, 开始定义卷积操作
        # 单个句子的训练是以[n_token, max_char, char_dim]
        # 作为一个训练样本，分别表示句长，单词最大字符长和每个字符维数。我们使用一个大小为[1, n_width, char_dim]
        # 的卷积核进行卷积，即高度为1，宽度为n_width，通道数为char_dim的卷积核进行卷积，每次都是一行一行的对n_width大小的字符进行卷积。
        # 卷积完之后，我们会形成一个[ n_token, max_char - n_width + 1]
        # 的feature
        # map图，然后我们再对feature
        ...
        def make_convolutions(inp, reuse):
            with tf.variable_scope('CNN', reuse=reuse) as scope:
                ...
                for i, (width, num) in enumerate(filters):
                    ...
                    #inp (batch_size, unroll_steps, max_chars, embed_dim)
                    #todo 输入大小为[batch, in_height, in_width, in_channels]
                    #todo [filter_height, filter_width, in_channels, out_channels]
                    conv = tf.nn.conv2d(
                            inp, w,
                            strides=[1, 1, 1, 1],
                            padding="VALID") + b
        ...
        #todo 开始进行highway网络和投影层， 要n_filters>proj_dim投影才有意义
        ...
        #todo 说白了 借助卷积的概念 将每个单词的字母进行卷积操作 最后确定为一个单词的embedding向量
    ...
    def _build(self):
        ...
        # LSTM options
        lstm_dim = self.options['lstm']['dim']#todo 4096
        projection_dim = self.options['lstm']['projection_dim']#todo 512
        n_lstm_layers = self.options['lstm'].get('n_layers', 1) #todo lstm的层数
        dropout = self.options['dropout']
        keep_prob = 1.0 - dropout

        if self.char_inputs:
            self._build_word_char_embeddings()#todo 创建字母的embedding模型，利用卷积的操作 将每个字母的embedding转换成单词的embedding
        else:
            self._build_word_embeddings() #todo 创建单词的embedding模型
        ...
        for lstm_num, lstm_input in enumerate(lstm_inputs):
            ...
            for i in range(n_lstm_layers):
                if projection_dim < lstm_dim:
                    ...
                    # pro_clip: 可选参数，float型，如果提供了num_proj > 0
                    # 和proj_clip，则投影值将元素裁剪到[-proj_clip, proj_clip]
                    # 范围
            ...
            #todo lstm_input的大小为[batch_size, unroll_steps, projection_dim]
...
def _get_feed_dict_from_X(X, start, end, model, char_inputs, bidirectional):
    ...
    :param end: 结束索引
    :param model: 模型
    :param char_inputs: 是否使用字符输入
    :param bidirectional: 是否使用双向循环神经网络
    :return:
    '''
    feed_dict = {}
    if not char_inputs:
...
def train(options, data, n_gpus, tf_save_dir, tf_log_dir,
        ...
        for k in range(n_gpus):
            with tf.device('/gpu:%d' % k):
                with tf.variable_scope('lm', reuse=k > 0):
                    ...
                    models.append(model)
                    # get gradients
                    #todo 计算<list of variables>相关的梯度
    ...
    with tf.Session(config=tf.ConfigProto(
        ...
        n_tokens_per_batch = batch_size * unroll_steps * n_gpus#todo 每一个epoch需要的数量
